...
$(document).ready(function () {
	...
	$(".select").on("click", function () {
		if ($(".select").hasClass("unchecked")) {
			...
			$(".select").css("border-top", "2px solid #30AE63").css("border-left", "2px solid #30AE63").css("border-right", "2px solid #30AE63").css("border-radius", 0);
		} else {
			...
			$(".select").css("border-top", "2px solid #dfe3e6").css("border-left", "2px solid #dfe3e6").css("border-right", "2px solid #dfe3e6").css("border-radius", "3px");
		}
	});
	$(".select").on("hover", function () {});
	$(".realty_search_content_select-item").on("click", function () {
		var selectedItem = $(this).text();
		let map = new Map([["Шулявская", 'red'], ["Золотые ворота", 'green'], ["Лыбедская", 'blue']]);
	});
